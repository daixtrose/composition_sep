...
{
    client_
        = create_client<example_interfaces::srv::AddTwoInts>("add_two_ints");
    // Note(dhood): The timer period must be greater than the duration of the
    // timer callback. Otherwise, the timer can starve a single-threaded
    // executor. See https://github.com/ros2/rclcpp/issues/392 for updates.
...
void Client::on_timer()
{
    ...
    // spin(). Unfortunately, the current version of spin() is not recursive and
    // so we cannot call spin() from within another spin(). Therefore, we cannot
    // wait for a response to the request we just made here within this
    ...
    // We then return from this callback so that the existing spin() function
    // can continue and our callback will get called once the response is
    // received.
    using ServiceResponseFuture
        = rclcpp::Client<example_interfaces::srv::AddTwoInts>::SharedFuture;
    auto response_received_callback = [this](ServiceResponseFuture future) {
        RCLCPP_INFO(
            this->get_logger(), "Got result: [%" PRId64 "]", future.get()->sum);
    };
    auto future_result
