...
// Create a Listener "component" that does not subclass the generic rclcpp::Node
// base class.
/**
 * Note that "components" don't have to derive from rclcpp::Node.
 * In the case that an object does not inherit from rclcpp::Node, then it must
 * implement:
 * - Constructor that takes `const rclcpp::NodeOptions&`
 * - get_node_base_interface() which returns a NodeBaseInterface::SharedPtr
 *
 * This is an example of an object that implements the interface required to be
 * a component.
 */
    ...
    : node_(std::make_shared<rclcpp::Node>("listener", options))
{
    ...
    auto callback
        ...
        RCLCPP_INFO(
            this->node_->get_logger(), "I heard: [%s]", msg->data.c_str());
        std::flush(std::cout);
    };
    ...
    sub_ = this->node_->create_subscription<std_msgs::msg::String>(
        "chatter", 10, callback);
}
